...
class RawClientTCP:
    def __init__(self):
        ...
        self.dest_port = None   # an arbitrary destination port
        self.ip_header = None
    ...
    def craft_tcp_header(self, tcp_seq=454, tcp_ack_seq=0, tcp_doff=5, tcp_fin=0, tcp_syn=0, tcp_rst=0,
        ...
        tcp_flags = (tcp_fin << 0) + (tcp_syn << 1) + (tcp_rst << 2) + (tcp_psh << 3) + (tcp_ack << 4) + (tcp_urg << 5)

        # the ! in the pack format string means network order
        tcp_header = pack('!HHLLBBHHH', self.source_port, self.dest_port, tcp_seq, tcp_ack_seq, tcp_offset_res,
                          tcp_flags, tcp_window, tcp_checksum, tcp_urg_ptr)

        # Pseudo header + TCP header + TCP payload
        psh = pack('!4s4sBBH', socket.inet_aton(self.source_ip), socket.inet_aton(self.dest_ip), 0, socket.IPPROTO_TCP, len(tcp_header) + len(self.msg))

        # Calculate TCP checksum
        tcp_checksum = self.calculate_checksum(psh + tcp_header + self.msg)
        
        # Make the TCP header again with correct checksum
        tcp_header = pack('!HHLLBBH', self.source_port, self.dest_port, tcp_seq, tcp_ack_seq, tcp_offset_res,
                          tcp_flags, tcp_window) + pack('<H', tcp_checksum) + pack('!H', tcp_urg_ptr)
    ...
    def craft_ack_packet(self):
        ...
        #here------------------------------------------------------------------------------------------------------------------------------
        ack_number = int(unpack("!I", self.received_packet[42:46])[0])

        self.ack_number = seq_number + 1
        self.seq_number = ack_number 

        self.craft_tcp_header(tcp_ack = 1, tcp_seq = self.seq_number, tcp_ack_seq = self.ack_number)
        self.packet_to_send = self.ip_header + self.tcp_header + self.msg
    ...
    def craft_msg_packet(self, msg):
        ...
        #here------------------------------------------------------------------------------------------------------------------------------
        self.msg = msg.encode()
        self.craft_tcp_header(tcp_psh=1, tcp_ack=1, tcp_seq=self.seq_number, tcp_ack_seq=self.ack_number)
        self.packet_to_send = self.ip_header + self.tcp_header + self.msg
